...
# Why Unit Tests Should Only Invoke SUT
...
{
  public GetPrice(Customer customer)
  {
...
- It is difficult to drive unit tests to cover each path through the `GetPrice` method.
...
Because the `PriceCalculator` takes in an interface, the test can control exactly how the dependancies work and this makes it super easy to drive the test.

In the previous we created a dummy implementation that drove the test. Creating lots of dummy implementations will quickly become unmanageable. This is where Mocking libraries come in.

---

# Mocks

A mock is a dummy implementation of an interface. `Moq` is by far the most popular mocking framework in .NET.
